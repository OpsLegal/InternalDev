...
| **Voice conversation** that waits until you're done: it keeps listening through pauses, waits twice as long when your sentence sounds unfinished ("...and", "...parce que"), and ends your turn on "go ahead" / "c'est tout", a tap on the mic, or the pause you chose. Answers are read aloud. After a question it listens again. Hands-free mode is optional | `app/.../voice/VoiceController.kt`, `core/.../voice/Turns.kt` |
| **Understands before acting**: it asks one question when something is unclear, then repeats what it understood. Changes are *staged* and only applied after your yes (button or voice). A "no" changes nothing | `core/.../agent/AgentTools.kt` (staging), `TdaAgent.confirm` |
| All of this is adjustable in **Settings → Talking with the assistant**: when to confirm (always / important changes only / never), asking when unsure, the pause length, "I'm done" phrases, yes/no words, language (EN/FR), voice speed, hands-free | `app/.../ui/SettingsScreen.kt` |
